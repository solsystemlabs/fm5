...
export function validateFiles(files: File[]): ValidationResult {
  ...
  for (const file of files) {
    ...
    if (!isImage && !isModel && !isArchive && !is3MF) {
      result.errors.push(`File "${file.name}" has unsupported type. Supported: images, model files, 3MF files, and ZIP archives.`);
    }
...
export function is3MFFile(file: File): boolean {
  ...
  return SUPPORTED_FILE_TYPES.threeMFFiles.extensions.includes(extension) ||
 ...
 */
export async function extract3MFImages(threeMFFile: File): Promise<ProcessedImageFile[]> {
  ...
  try {
    ...
    for (const [filename, zipEntry] of Object.entries(zipData.files)) {
      ...
      if (zipEntry.dir || !filename.toLowerCase().match(/\.(png|jpg|jpeg|gif|bmp|webp)$/i)) {
        continue;
      }

      // 3MF files commonly have thumbnails in specific paths
      const is3MFThumbnail = filename.includes('Metadata') || 
...
export async function processUploadedFiles(
  ...
  const otherFiles = files.filter(file => !isArchiveFile(file) && !is3MFFile(file));
