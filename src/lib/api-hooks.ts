...
import { FilamentType, type ModelFile } from "@prisma/client";
...
const QUERY_KEYS = {
  ...
  modelFilesByModel: (modelId: number) =>
    ["modelFiles", "byModel", modelId] as const,
};
  ...
  return useMutation({
    ...
    onSuccess: (newBrand) => {
      ...
      queryClient.setQueryData(QUERY_KEYS.brands, (oldBrands: Brand[] = []) => {
        return [...oldBrands, newBrand].sort((a, b) =>
  ...
  return useMutation({
    ...
    onSuccess: (newMaterialType) => {
      ...
      queryClient.setQueryData(
        QUERY_KEYS.materialTypes,
        (oldTypes: MaterialType[] = []) => {
          return [...oldTypes, newMaterialType].sort((a, b) =>
            a.name.localeCompare(b.name),
          );
        },
      );
...
export function useCreateFilamentType() {
  ...
  return useMutation({
    ...
    onSuccess: (newFilamentType) => {
      ...
      queryClient.setQueryData(
        QUERY_KEYS.filamentTypes,
        (oldTypes: FilamentType[] = []) => {
          return [...oldTypes, newFilamentType].sort((a, b) =>
            a.name.localeCompare(b.name),
          );
        },
      );
...
export function useCreateModelCategory() {
  ...
  return useMutation({
    ...
    onSuccess: (newModelCategory) => {
      ...
      queryClient.setQueryData(
        QUERY_KEYS.modelCategories,
        (oldCategories: ModelCategory[] = []) => {
          return [...oldCategories, newModelCategory].sort((a, b) =>
            a.name.localeCompare(b.name),
          );
        },
      );
...
export function useUploadSlicedFileWithProgress() {
  ...
  return {
    uploadWithProgress: (
      ...
      return api
        .uploadSlicedFileWithProgress(formData, onProgress)
        .then((result) => {
          // Invalidate and refetch sliced files after uploading a new one
          queryClient.invalidateQueries({ queryKey: QUERY_KEYS.slicedFiles });
          // Update dashboard analytics
...
export function useUploadModelFilesWithProgress() {
  ...
  return {
    uploadWithProgress: (
      ...
      return api
        .uploadModelFilesWithProgress(modelId, formData, onProgress)
        .then((result) => {
          // Invalidate relevant queries after uploading files
          queryClient.invalidateQueries({ queryKey: QUERY_KEYS.models });
          queryClient.invalidateQueries({ queryKey: QUERY_KEYS.modelFiles });
