...
- [ ] **Task 2: Resend Email Service Configuration (AC: 2)**
...
- [ ] **Task 3: Service Monitoring and Health Checks (AC: 3)**
  - [ ] Implement health check endpoints for R2 and email services
  - [ ] Configure health monitoring for database, R2 storage, and email services
...
- [ ] **Task 4: Frontend External Service Testing Interface (AC: 4)**
  - [ ] Add environment gating to prevent `/dev/*` routes in production (throw error or 404)
...
- [ ] **Task 5: Environment Health Dashboard (AC: 5)**
  - [ ] Add environment gating for health dashboard (dev/staging only)
  - [ ] Build environment health status dashboard showing staging vs production
  - [ ] Implement real-time service status indicators (R2, Resend, database)
  - [ ] Create environment-specific health check endpoints with detailed status reporting
  - [ ] Add service response time monitoring and visual status display
  - [ ] Create environment comparison view for service configuration validation

- [ ] **Task 6: End-to-End Integration Testing Workflow (AC: 6)**
