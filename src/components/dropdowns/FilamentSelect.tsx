...
export default function FilamentSelect({
  ...
  const allFilaments = groupedFilaments
    ? Object.values(groupedFilaments).flat()
    : [];

  // Filter filaments based on search
  const filteredGroupedFilaments = groupedFilaments
    ? Object.entries(groupedFilaments).reduce(
        (acc, [typeName, filaments]) => {
        ...
        {} as typeof groupedFilaments,
      )
    : null;

  // Get selected filaments for display
  const selectedFilaments = allFilaments.filter(
    (filament) =>
      ...
      <DialogTrigger>
          ...
          {selectedFilaments.length === 0 ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="truncate">
                {selectedFilaments[0].name} - {selectedFilaments[0].Brand.name}
              </span>
            </div>
          ) : (
            <span className="truncate">
              {selectedFilaments.length} filaments selected
            </span>
          )}
          <ChevronDownIcon
        ...
        <Popover className="ring-opacity-5 bg-popover border-border mt-1 max-h-60 w-[var(--trigger-width)] overflow-auto rounded-md border py-1 text-base shadow-lg ring-1 ring-black focus:outline-none sm:text-sm">
          <SearchField
            ...
            className="group border-input mx-2 mt-2 flex items-center rounded-md border"
            value={searchValue}
          ...
          >
            ...
            <Button
              className="text-muted-foreground hover:text-foreground mr-1 flex w-6 items-center justify-center rounded-md border-0 bg-transparent p-1 text-center text-sm transition group-empty:invisible"
          ...
          >
            ...
            {filteredGroupedFilaments &&
              Object.keys(filteredGroupedFilaments).length === 0 && (
                <ListBoxItem className="text-muted-foreground relative cursor-default py-2 pr-9 pl-3 select-none">
                  {searchValue
                    ? `No filaments found matching "${searchValue}"`
                    : "No filaments available"}
                </ListBoxItem>
              )}
            {filteredGroupedFilaments &&
              Object.entries(filteredGroupedFilaments).map(
                ([filamentTypeName, filaments]) => (
                  <ListBoxSection key={filamentTypeName}>
                    <Header className="text-muted-foreground border-border border-b px-3 py-2 text-xs font-semibold tracking-wide uppercase">
                      {filamentTypeName}
                    </Header>
